$(function(){
    ...
    var changeBgImageDim=function(){
        ...
        var $img=new Image();
        $img.src=$i.attr('src');
        var w_w=$(window).width(),w_h=$(window).height(),r_w=w_h/w_w,i_w=$img.width,i_h=$img.height,r_i=i_h/i_w,new_w,new_h,new_left,new_top;
        if(r_w>r_i){
            new_h=w_h;
            new_w=w_h/r_i
            }else{
    ...
    return $.Deferred(function(dfd){
        ...
        $bf_items.each(function(i){
            var $item=$(this),$itemImg=$item.children('img'),itemImgSrc=$itemImg.attr('src'),itemBgImgSrc=$itemImg.data('bgimg');
...
return{
    ...
    var animated=false,animSpeed=700,current=0,defaultImage='images/background/default.jpg',init=function(position){
        initEventsHandler();
        slider(position)
        },stop=function(){
        ...
        $bf_close.bind('click',function(e){
            hideNavigation();
            light(500,0.5);
            var $bf_item_current=$bf_items.eq(current),$heading=$bf_item_current.children('.bf_heading'),$desc=$bf_item_current.children('.bf_desc');
            animated=true;
            stop();
        ...
        $(document).bind('mousewheel',function(e,delta){
            if(delta>0)navigate(0);else navigate(1);
            return false
            }).keydown(function(e){
            switch(e.which){
                case 37:
    ...
    var $next_item=$bf_items.eq(current),$headingCurrent=$bf_item_current.children('.bf_heading'),$descCurrent=$bf_item_current.children('.bf_desc');
    $.when(animateHeadingDesc($headingCurrent,$descCurrent,0)).done(function(){
        $bf_gallery_wrapper.css('overflow','hidden');
        var startingLeft=(dir)?'480px':'-520px',currentEndingLeft=(dir)?'-520px':'480px';
        ...
        $next_item.stop().animate({
            ...
            var $heading=$next_item.children('.bf_heading'),$desc=$next_item.children('.bf_desc');
    ...
    var $next_item_bgimage=$('<img src="'+$next_item.children('img').data('bgimg')+'" alt="image'+(current+1)+'" ></img>'),dim=BGImageController.getImageDim($next_item_bgimage),starting_left=(dir)?$(window).width():-dim.width+'px';
    ...
    var $next_bgimage=$('<img src="'+image+'" alt="image'+(current+1)+'" ></img>'),dim=BGImageController.getImageDim($next_bgimage);
    $next_bgimage.css({
        width:dim.width+'px',
        height:dim.height+'px',
        left:dim.left,
...
return{
    ...
    var animSpeed=700,init=function(){
